...
function WholesalerSignup({ onNavigate }) {
  ...
  const handleSignup = async (e) => {
    ...
    if (Object.keys(newErrors).length === 0) {
      setLoading(true);
      try {
        const response = await signup(name, email, password, "wholesaler", businessName);
        console.log("Signup response:", response);
        alert("Wholesaler signup successful! Please login.");
        // Use onNavigate to go to login
        onNavigate && onNavigate('login', 'wholesaler');
      } catch(err) {
      ...
      <form className="login-form" onSubmit={handleSignup}>
        ...
        <div style={{ marginBottom: '20px' }}>
          ...
          <input 
            ...
            style={{
              ...
              padding: '14px',
              borderRadius: '8px',
              border: `2px solid ${errors.businessName ? '#ff6b6b' : '#b0d4e3'}`,
              fontSize: '1rem',
              boxSizing: 'border-box',
              transition: 'border-color 0.3s'
            }}
          />
          {errors.businessName && <div className="error-message" style={{ color: '#ff6b6b', marginTop: '8px' }}>{errors.businessName}</div>}
        </div>
        ...
        <div className="link" style={{ textAlign: 'center', marginTop: '25px' }}>
          <p style={{ color: '#0a4f70' }}>
            ...
            <span 
              onClick={() => onNavigate && onNavigate('login', 'wholesaler')}
              style={{ 
                color: '#00a8cc', 
                textDecoration: 'none', 
                fontWeight: 'bold',
