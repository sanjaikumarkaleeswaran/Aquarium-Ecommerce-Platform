...
function RetailerLogin({ onNavigate }) {
    ...
    if (Object.keys(newErrors).length === 0) {
      try {
        ...
        if (res.user.role !== 'retailer') {
          setModalContent({
            title: 'Access Denied',
            message: 'Retailer access required. Please use the correct login page for your role.'
          });
          setShowModal(true);
          return;
        }
        ...
        onNavigate && onNavigate('dashboard', 'retailer');
      ...
      <form className="login-form" onSubmit={handleLogin}>
        ...
        <div className="link" style={{ textAlign: 'center', marginTop: '25px' }}>
          <p style={{ color: '#0a4f70' }}>
            ...
            <span
              onClick={() => onNavigate && onNavigate('signup', 'retailer')}
              style={{
                color: '#00a8cc',
                textDecoration: 'none',
                fontWeight: 'bold',
